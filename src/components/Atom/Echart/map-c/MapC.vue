<template>
  <v-chart :update-options="{notMerge:true}" :option="option" :initOptions="initOptions" :theme="theme" autoresize ref="myChart" />
</template>
<script>
...
export default {
  ...
  mounted() {
    registerMap(this.getDetail.mapName, JSON.parse(JSON.stringify(this.getDetail.mapObj)));
  },
  data() {
    return {
